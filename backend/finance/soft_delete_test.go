...
func TestSoftDeleteRestoreRoundTrip(t *testing.T) {
	...
	exRes := s.CreateExpense(ctx, period+"-15", "Super", "Comida", "", &card.ID, KindUnico, "10000", 1)
	if exRes.Error != nil {
...
func TestDeleteCardKeepsHistoricalCardName(t *testing.T) {
	...
	period := currentPeriod()
	exRes := s.CreateExpense(ctx, period+"-10", "Compra", "General", "", &card.ID, KindUnico, "20000", 1)
	if exRes.Error != nil {
		t.Fatalf("CreateExpense: %v", exRes.Error)
	}
...
func TestDeleteExpenseExcludedFromSummaryTotals(t *testing.T) {
	...
	exRes := s.CreateExpense(ctx, period+"-05", "Compra suelta", "General", "", nil, KindUnico, "15000", 1)
	if exRes.Error != nil {
		t.Fatalf("CreateExpense: %v", exRes.Error)
	}
